	...
	useEffect(() => {
		async function fetchAirtableData() {
			try {
				...
				const isFavouriteRecipe = airtableRecipes.some(
					(recipe) => recipe.fields.recipeid === id
				);
				setIsFavourite(isFavouriteRecipe);
				if (isFavouriteRecipe) {
					const airtableRecord = airtableRecipes.find(
						(recipe) => recipe.fields.recipeid === id
					);
		...
		>
			<div className="container h-100" style={{ minHeight: '100%' }}>
				...
				<div className="row overflow-hidden">
					...
					<div
						className="card border-primary mt-4 mb-3"
					...
					<div className="row px-0">
						...
						>
							...
							<ul className="offset-md-1" style={{ fontSize: '18px' }}>
						...
						>
							...
							{recipe.analyzedInstructions[0].steps.map((recipeStep, index) => (
								<div
									className="mt-2 offset-md-1 pr-5"
									key={index}
									style={{ fontSize: '18px' }}
								>
